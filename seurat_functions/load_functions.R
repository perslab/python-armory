...
print("Started loading Pers lab Seurat functions...")

dir.seurat_functions = LocationOfThisScript() # get the directory where this file lives. 
print(sprintf("Seurat function dir: %s", dir.seurat_functions))

files.seurat_functions <- list.files(dir.seurat_functions, pattern="*.R") # get all .R files in the current directory as a vector. Returns basename (e.g. "plot_seurat.R") and not full path

for (file2source in files.seurat_functions) {
  if (file2source == SCRIPT_NAME) { # don't source this file. We need this to avoid ending up in a infinity 'file sourcing' loop!
    next
  }
  tmp.filepath <- file.path(dir.seurat_functions, file2source) # creating absolute path
  source(tmp.filepath)
  print(sprintf("Sourced file: %s", tmp.filepath))
}

print("Done loading Pers lab Seurat functions")
