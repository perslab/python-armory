...
setwd("/home/cbmr/djw472/ygg-projects/timshel/sc-arc_lira/src") # let's not save the 'wd' variable
...
analysis_prefix <- "lira_vs_pf" # will be used as prefix for output files.
...
rm(list=c(matrix.dat1, matrix.dat2))
...
# ====== SAVE IMAGE ======
save.image(file=sprintf("%s-rsession2_after_alignment.RData", analysis_prefix))
# ========================
...
#Identify Markers
df.all_markers <- FindAllMarkers(seurat_merge.align, only.pos = TRUE, min.pct = 0.25,  thresh.use = 0.25) # TAKES A LONG TIME
file.out.tmp <- sprintf("%s-all_markers-res_%s.csv", analysis_prefix, res.primary)
write.csv(df.all_markers, file=file.out.tmp, row.names=F, quote=F)
...
list_of_dfs.conserved_markers <-parLapply(cl, x, function(x) FindConservedMarkers(seurat_merge.align, ident.1 = x, grouping.var="condition")) # returns a list of data frames (not named - only 'indexes')
stopCluster(cl)
...
# Subset seurat object into clusters
cluster_ids <- sort(unique(seurat_merge.align@meta.data[,paste0("res.",res.primary)])) # numeric vector, 0, 1, 2, 3, ... (N_CLUSTERS-1)
...
# names(list_of_seurat_objs_per_cluster) <- cluster_ids # not needed?
...
# Subset seurat object into clusters
cluster_ids.pre_align <- sort(unique(seurat_merge.pre_align@meta.data[,paste0("res.",res.primary)])) # numeric vector, 0, 1, 2, 3, ... (N_CLUSTERS-1)
