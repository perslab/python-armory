...
# time Rscript align_datasets.R |& tee align_datasets.out.txt
  # Bash version > 4
...
FLAG_TEST_RUN <- FALSE # set to TRUE to load only 500 cells and a limited number of highly expressed genes
...
df.cluster_markers <- bind_rows(list_of_dfs.all_markers,.id="cluster_no") # combine list of dfs into a single data frame
...
stopCluster(cl)
names(list_of_dfs.condition_de_genes_per_cluster) <- cluster_ids # NOT IMPORANT (although lapply will have wrong 'offset' for cluster_no, i.e. 1, 2, 3 instead of 0, 1, 2, ..): adding names before calling bind_rows. In this way we ensure that the cluster information is kept. this works even if condition_de_genes returns one or more "NULL" values.
...
stopCluster(cl)
names(list_of_dfs.condition_de_genes_per_cluster.pre_align) <- cluster_ids.pre_align # NOT IMPORANT (although lapply will have wrong 'offset' for cluster_no, i.e. 1, 2, 3 instead of 0, 1, 2, ..): adding names before calling bind_rows. In this way we ensure that the cluster information is kept. this works even if condition_de_genes returns one or more "NULL" values.
df.condition_de_genes_per_cluster.pre_align <- bind_rows(list_of_dfs.condition_de_genes_per_cluster.pre_align, .id="cluster_no") # returns results for all genes (no p_val_adj filter)


file.out.tmp <- sprintf("%s-condition_de_genes_per_cluster_pre_align-res_%s.csv", analysis_prefix, res.primary)
